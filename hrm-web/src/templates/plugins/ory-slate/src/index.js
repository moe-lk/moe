...
export default (plugins: Plugin[] = hooks.defaultPlugins) => {
  ...
  const BarButtons = ({ editorState, onChange, focus }: Props) => (
    <div>
      {plugins.map((plugin: Plugin, i: number) =>
        plugin.barButtons.map((Button: any, j: number) => (
          <Button
            key={`${i}-${j}`}
            editorState={editorState}
            onChange={onChange}
  ...
  return {
    ...
    text: 'Custom Text',
    description: 'An advanced rich text area with.',
